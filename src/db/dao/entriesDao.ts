...
const rowToEntry = (row: JournalEntryRow): JournalEntry => {
  ...
  let parsedAmplitude: number[] | undefined;
  if (row.amplitude_data) {
    try {
      const parsed = JSON.parse(row.amplitude_data);
      if (Array.isArray(parsed)) {
        parsedAmplitude = parsed.map(Number);
      }
    } catch {
      // Ignore
    }
  ...
  return {
    ...
    amplitude_data: parsedAmplitude,
  };
};
...
export const entriesDao = {
  async insertEntry(entry: JournalEntry): Promise<void> {
    ...
    const amplitudeJson = entry.amplitude_data
      ? JSON.stringify(entry.amplitude_data)
      : null;
    await db.runAsync(
      `INSERT INTO entries (
